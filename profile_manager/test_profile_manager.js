...
module.exports = function (RED) {
    function ProfileManager(config) {
        ...
        const nodeConfig = {
            ...
            profileNameOrId: config.profileNameOrId || '' // New config option for profile selection
        };
        ...
        function selectProfile(profiles, nameOrId) {
            if (!nameOrId) {
                return profiles[0]; // Default to first profile if no name/id provided
            }
            
            const selectedProfile = profiles.find(profile => 
                profile.name === nameOrId || profile.id === nameOrId // Edit to match lower_case()
            );

            if (!selectedProfile) {
                throw new Error(`No profile found with name or id: ${nameOrId}`);
            }
        ...
        this.on("input", async function (msg, send, done) {
            try {
                ...
                // Use the profileNameOrId from nodeConfig
                const selectedProfile = selectProfile(profiles, nodeConfig.profileNameOrId);
    ...
    RED.nodes.registerType("profile-manager", ProfileManager, {
        defaults: {
            ...
            profileNameOrId: { value: "" } // New config option for profile selection
        }
    });
};